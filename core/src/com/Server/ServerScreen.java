...
public class ServerScreen implements Screen {
    ...
    public ArrayList<Bullet> bulletArr = new ArrayList<>();
    ...
    public ServerScreen(TDGame game) {
        this.game = game;
        ...
        Globals.serverScreen = this;
        if(Globals.server == null){
            Globals.server = new ServerThread(this);
    ...
    private void handleInput() {
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_1)) {
            anySelected = new MoneySlime(0, 0, money, true);
        }else if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_2)) {
            anySelected = new ShooterSlime(0,0,bulletArr,true);
        }else if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_3)) {
            anySelected = new ShieldSlime(0,0,true);
        } else if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_8)) {
            anySelected = new BasicBoulder(0, 0, true);
        }else if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_9)) {
            anySelected = new FastBoulder(0,0,true);
        }else if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_0)) {
            anySelected = new ArmoredBoulder(0,0,true);
        }
    ...
    public void render(float delta) {
        ...
        if(!Globals.server.ready) return;
    ...
    private void updateTroops() {
        for (BaseTroop troop : troopArr) {
            if (troop instanceof Slime) {
                ...
                if(troop.hitbox.x<=-2&&troop.getHp()>0){
                    over(1);
                }
            }
        }
    ...
    public void updateTimer(float delta) {
        if (HUD != null) {
            int tempTime = HUD.getTime();
            HUD.update(game, delta);
            if(tempTime != HUD.getTime()) {
                Globals.server.broadcastEvent("timer#"+HUD.getTime());
            }
            if (HUD.getTime() <= 0) {
                HUD.stop();
                over(0);
    ...
    public void moneyFactory(){
        moneyCounter += Gdx.graphics.getDeltaTime();
        if(moneyCounter >= 3) {
            System.out.println("m");
            for(BaseTroop t : troopArr){
                if(t instanceof MoneySlime) Globals.player0Money+=2;
            }
            Globals.player0Money += 5;
            Globals.player1Money += 5;
            if(Globals.server.getClients().get(0) != null) Globals.server.sendEvent("money#"+Globals.player0Money, Globals.server.getClients().get(0));
            if(Globals.server.getClients().get(1) != null) Globals.server.sendEvent("money#"+Globals.player1Money, Globals.server.getClients().get(1));
            moneyCounter = 0;
        }
    }

    public void over(int winner){
        Globals.server.broadcastEvent("over#"+winner);
